  ...
  return (
    <html lang="en" className={`${pixel.variable} ${body.variable}`}>
      <head>
        ...
        />
        {/* Analytics · load on first interaction, otherwise well after the
            critical PageSpeed window. GA/Clarity were the largest mobile TBT
            tasks even with lazyOnload. */}
        {(CLARITY_ID || GA_MEASUREMENT_ID) && (
          <Script id="peppudex-delayed-analytics" strategy="lazyOnload">
            {`
              (function(){
                var loaded=false;
                function load(){
